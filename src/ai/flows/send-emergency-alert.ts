...
const SendEmergencyAlertOutputSchema = z.object({
  confirmationMessage: z.string().describe('A confirmation that the alert was processed.'),
...
const prompt = ai.definePrompt({
  ...
  prompt: `
    You are the CampusGuard emergency alert system. Your task is to generate a professional and clear emergency alert email based on the provided information.

    The alert is for: {{{zoneName}}}
    The core message is: {{{customMessage}}}

    1.  Create a clear and concise email subject line. Start it with "EMERGENCY ALERT:".
    ...
    3.  Generate a confirmation message indicating the action has been taken.
  `,
});
